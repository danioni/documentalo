...
<div className="mb-8 flex flex-wrap gap-2">
  <span className="rounded-full border border-amber-500/30 bg-amber-500/10 px-3 py-1 text-xs font-medium text-amber-400">Borrador v0.10</span>
  <span className="rounded-full border border-cyan-500/30 bg-cyan-500/10 px-3 py-1 text-xs font-medium text-cyan-400">Compatible con MCP</span>
  <span className="rounded-full border border-gray-600 bg-gray-800 px-3 py-1 text-xs font-medium text-gray-300">Estándar abierto</span>
</div>
...
Servicialo define un **conjunto estandarizado de operaciones** para gestionar servicios profesionales: agendamiento, pagos, clientes y notificaciones.
...
| **Nativo para IA** | Diseñado para ser consumido por agentes de IA vía MCP |
...
Servicialo define cuatro grupos de herramientas que toda implementación debe exponer. **Todas las herramientas reciben `actor` como parámetro requerido** para registrar quién inicia cada operación.

### Agendamiento (Scheduling)

Herramientas para gestionar el ciclo de vida de las sesiones.
...
Herramientas para gestionar el flujo financiero asociado a los servicios.
...
Herramientas para gestionar el directorio de clientes de la organización.
...
Herramientas para comunicaciones con clientes a través de múltiples canales.
...
Derivaciones entre organizaciones y profesionales.
...
- **Calificaciones**: Puntuación bidireccional (cliente ↔ profesional)
...
Servicialo soporta relaciones de muchos a muchos entre las entidades principales:
...
<div className="flex items-center gap-2 mb-4">
  <span className="rounded-full bg-green-500/10 border border-green-500/30 px-3 py-1 text-xs font-medium text-green-400">En producción</span>
</div>
...
- **Cobertura**: Agendamiento, Pagos, Clientes y Notificaciones
- Versión del protocolo declarada por Coordinalo: 0.9 (manifest); protocolo vigente: 0.10 (borrador)
- **Entorno**: Producción
...
Servicialo expone sus herramientas como un servidor MCP, permitiendo que agentes de IA interactúen con servicios profesionales de forma nativa.

**Namespace**: `com.servicialo/mcp-server`
...
### Ejemplo: Agendar una cita vía agente de IA

Un agente de IA puede usar las herramientas de Servicialo para gestionar sesiones. Este es el flujo completo para agendar una cita:
...
- [Especificación (v0.10, borrador)](https://servicialo.com/spec)
- [GitHub — Servicialo](https://github.com/servicialo/mcp-server)
- [GitHub — Documentalo](https://github.com/danioni/documentalo)
